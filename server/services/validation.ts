...
export function validateOutput(raw: string, schemaName: string): { success: boolean; data?: any; error?: string } {
  try {
    let clean = raw.trim();
...
export function checkRelevance(prompt: string, response: string): boolean {
  if (!prompt || !response) return true;
