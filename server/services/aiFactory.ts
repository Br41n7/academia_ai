...
export async function executeAIGeneration(
  preferredModel: 'gemini' | 'deepseek' | 'claude' | 'openai',
  prompt: string,
  systemInstruction: string,
  responseMimeType?: string,
  keys?: { openaiKey?: string; anthropicKey?: string; deepseekKey?: string },
  image?: { base64: string; mimeType: string }
): Promise<string> {
  const baseOrder: ('gemini' | 'deepseek' | 'claude' | 'openai')[] = ['gemini', 'deepseek', 'claude', 'openai'];
