...
async function getProjectContextServer(projectId, userId) {
    try {
 ...
 */
router.post('/generate', authenticateToken, rateLimit, creditCheck, async (req, res) => {
    try {
        const { prompt, systemInstruction = '', responseMimeType, modelPreference = 'gemini', persona = '', region = 'default', originalPrompt = '', validationType, image, } = req.body;
        if (!prompt) {
        ...
        let responseText = await executeAIGeneration(modelPreference, prompt, systemPrompt, responseMimeType, req.userApiKeys, image);
        // 3. Strip fences
        ...
        if (!validationPassed) {
            ...
            responseText = await executeAIGeneration(modelPreference, strictPrompt, systemPrompt, responseMimeType, req.userApiKeys, image);
...
// ─────────────────────────────────────────────────────────────────────────────
router.post('/notebook-action', authenticateToken, rateLimit, creditCheck, async (req, res) => {
    try {
        ...
        const responseText = await executeAIGeneration('gemini', prompt, 'You are a highly precise learning copilot.', 'text', req.userApiKeys);
...
router.post('/notebook/action', authenticateToken, rateLimit, creditCheck, async (req, res) => {
    ...
    try {
        ...
        const responseText = await executeAIGeneration('gemini', prompt, 'You are a helpful study companion.', 'text', req.userApiKeys);
...
router.post('/tune-question', authenticateToken, rateLimit, creditCheck, async (req, res) => {
    try {
...
Return your answer strictly in JSON matching: { "improved_question": "string" }`;
        const responseText = await executeAIGeneration('gemini', prompt, 'You are a helpful socratic study coach.', 'application/json', req.userApiKeys);
...
router.post('/generate-quiz', authenticateToken, rateLimit, creditCheck, async (req, res) => {
    try {
...
IMPORTANT: Provide explanations for correct and incorrect options. Return response in valid JSON matching 'quiz' schema format.`;
        const responseText = await executeAIGeneration('gemini', prompt, `You are a strict examiner. Format: {"questions": [{"id": "q1", "type": "multiple_choice", "question": "...", "options": ["a", "b"], "correct_answer": "a", "explanation": "..."}]}`, 'application/json', req.userApiKeys);
...
router.post('/generate-mnemonic', authenticateToken, rateLimit, creditCheck, async (req, res) => {
    try {
        const { concept, style, projectId } = req.body;
        const context = await getProjectContextServer(projectId, req.user?.uid || '');
        const prompt = `Generate a mnemonic for "${concept}" in style "${style}" based on context:\n${context}`;
        const responseText = await executeAIGeneration('gemini', prompt, 'Return format: {"mnemonic": "...", "explanation": "..."}', 'application/json', req.userApiKeys);
...
router.post('/check-plagiarism', authenticateToken, rateLimit, async (req, res) => {
    try {
        const { text } = req.body;
        const prompt = `Analyze this text for potential plagiarism: "${text}"`;
        const responseText = await executeAIGeneration('gemini', prompt, 'Return format: {"similarity_score": 0, "analysis_summary": "...", "sources": []}', 'application/json', req.userApiKeys);
        return res.json(JSON.parse(stripFences(responseText)));
    }
    catch (err) {
        return res.status(500).json({ error: err.message });
...
router.post('/generate-visual', authenticateToken, rateLimit, creditCheck, async (req, res) => {
    try {
        ...
        const responseText = await executeAIGeneration('gemini', prompt, 'Return JSON schema representing the requested visual format.', 'application/json', req.userApiKeys);
        return res.json(JSON.parse(stripFences(responseText)));
    }
    catch (err) {
        return res.status(500).json({ error: err.message });
...
router.post('/generate-exam', authenticateToken, rateLimit, creditCheck, async (req, res) => {
    try {
        const { projectId, config } = req.body;
        const context = await getProjectContextServer(projectId, req.user?.uid || '');
        const prompt = `Generate a comprehensive exam based on this context:\n${context}\nConfig: ${JSON.stringify(config)}`;
        const responseText = await executeAIGeneration('gemini', prompt, 'Return JSON matching standard exam schema containing lists of questions.', 'application/json', req.userApiKeys);
        return res.json(JSON.parse(stripFences(responseText)));
    }
    catch (err) {
        return res.status(500).json({ error: err.message });
...
router.post('/analyze-exam', authenticateToken, rateLimit, async (req, res) => {
    try {
        const { exam, answers } = req.body;
        const prompt = `Analyze performance. Exam: ${JSON.stringify(exam)}, Answers: ${JSON.stringify(answers)}`;
        const responseText = await executeAIGeneration('gemini', prompt, 'Return JSON matching performance analysis schema.', 'application/json', req.userApiKeys);
        return res.json(JSON.parse(stripFences(responseText)));
    }
    catch (err) {
        return res.status(500).json({ error: err.message });
...
router.post('/generate-course-ai', authenticateToken, rateLimit, creditCheck, async (req, res) => {
    try {
        const { projectId } = req.body;
        const context = await getProjectContextServer(projectId, req.user?.uid || '');
        const prompt = `Create structured course based on content:\n${context}`;
        const responseText = await executeAIGeneration('gemini', prompt, 'Return JSON with structured syllabus containing modules and lessons.', 'application/json', req.userApiKeys);
...
router.post('/generate-study-guide', authenticateToken, rateLimit, creditCheck, async (req, res) => {
    try {
        const { projectId } = req.body;
        const context = await getProjectContextServer(projectId, req.user?.uid || '');
        const prompt = `Generate comprehensive study guide Markdown for context:\n${context}`;
        const responseText = await executeAIGeneration('gemini', prompt, 'You are a brilliant study guide writer.', 'text', req.userApiKeys);
...
router.post('/generate-slide-deck', authenticateToken, rateLimit, creditCheck, async (req, res) => {
    try {
        const { projectId } = req.body;
        const context = await getProjectContextServer(projectId, req.user?.uid || '');
        const prompt = `Generate a slide deck based on context:\n${context}`;
        const responseText = await executeAIGeneration('gemini', prompt, 'Return slide deck as a JSON array of slide objects.', 'application/json', req.userApiKeys);
        return res.json(JSON.parse(stripFences(responseText)));
    }
    catch (err) {
        return res.status(500).json({ error: err.message });
...
router.post('/analyze-confusion', authenticateToken, rateLimit, async (req, res) => {
    try {
        const { query, projectId } = req.body;
        const context = await getProjectContextServer(projectId, req.user?.uid || '');
        const prompt = `The user is confused about: "${query}". Based on context, provide simplified explanations:\n${context}`;
        const responseText = await executeAIGeneration('gemini', prompt, 'Include simple analogies and step-by-step breakdowns.', 'text', req.userApiKeys);
        return res.json({ text: responseText, result: responseText });
    }
    catch (err) {
        return res.status(500).json({ error: err.message });
...
router.post('/concept-battle', authenticateToken, rateLimit, creditCheck, async (req, res) => {
    try {
        ...
        const responseText = await executeAIGeneration('gemini', prompt, 'Return concept battle comparison JSON structure.', 'application/json', req.userApiKeys);
        return res.json(JSON.parse(stripFences(responseText)));
    }
    catch (err) {
        return res.status(500).json({ error: err.message });
